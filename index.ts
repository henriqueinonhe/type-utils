...
> = Target extends unknown
  ? Omit<Target, keyof Source> & Pick<Source, keyof Source & DistributedKeyOf<Target>>
  : never;

export type ArrayType<T extends Array<unknown>> = T extends Array<infer U> ? U : never;

export type ExtractTypeFromTypeGuard<T extends (value: unknown) => value is unknown> = T extends (
  value: unknown
) => value is infer U
  ? U
  : never;
...
export type DistributedOmit<T, K extends KeyLike> = T extends unknown ? Omit<T, K> : never;
...
export type PickAllKeys<T, K extends DistributedKeyOf<T>> = Pick<UnionAllKeys<T>, K>;
